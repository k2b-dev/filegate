...
func allConfigFlagSpecs() []configFlagSpec {
	return []configFlagSpec{
		...
		{Name: "auth-bearer-token", Path: "auth.bearer_token", Kind: configFlagString, Usage: "REST bearer token", Scope: scopeStatic, Reason: "loaded at startup so the break-glass credential remains available when the runtime store is damaged", Secret: true},
		...
		{Name: "s3-access-key", Path: "s3.access_key", Kind: configFlagString, Usage: "single-key S3 seed access key", Scope: scopeStatic, Reason: "credentials are seeded into the resource store during S3 listener startup", Secret: true},
		{Name: "s3-secret-key", Path: "s3.secret_key", Kind: configFlagString, Usage: "single-key S3 seed secret key", Scope: scopeStatic, Reason: "credentials are seeded into the resource store during S3 listener startup", Secret: true},
